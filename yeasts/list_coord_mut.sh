...
if [[ -n "$all_dir" ]]; then
 for dir in ${all_dir}; do
  ...
  if [[ -d "${dir}/coord" ]]; then rm -rf "${dir}/coord"; fi
  mkdir -p "${dir}/coord"
  ...
  if [[ "$count_name" -ge 2 ]]; then
   ...
   (
   "${path_exec}/list_coord_mut" "${dir}/common/${ancestor}-${ref_name}_common_control_mpileup.dat" "${dir}/common/${endpoint}-${ref_name}_common_evolved_mpileup.dat" "${dir}/coord/" "${modal_ancestor}" "${std_ancestor}" "${modal_endpoint}" "${std_endpoint}" "${ploidy}"
   )
  fi
